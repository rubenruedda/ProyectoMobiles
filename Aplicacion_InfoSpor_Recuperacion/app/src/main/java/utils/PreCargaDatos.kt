...
class PreCargaDatos(private val context: Context) {

    private val db = AppDataBase.getDatabase(context)

    fun cargarDatos() {
        CoroutineScope(Dispatchers.IO).launch {
            val ligasExistentes = db.ligaDao().obtenerLigasList()

            // Ahora sí funciona el isEmpty() porque es una lista real
            if (ligasExistentes.isEmpty()) {
                insertarLigas()
                insertarPartidos()
                insertarNoticias()
            }
    ...
    private suspend fun insertarLigas() {
        val ligas = listOf(
        ...
        // Insertamos una a una o usas insertAll si lo tienes definido
        ligas.forEach { db.ligaDao().insertarLigas(ligas) }
    }

    private suspend fun insertarPartidos() {
        val partidos = listOf(
        ...
        partidos.forEach { db.partidoDao().insertarPartidos(partidos) }
    }

    private suspend fun insertarNoticias() {
        val noticias = listOf(
        ...
        noticias.forEach { db.noticiaDao().insertarNoticias(noticias) }
    }

    private suspend fun insertarEquipos(){
        val equipos = listOf(
            // España
        ...
        equipos.forEach { db.ligaDao().insertarEquipos(equipos) }
    }

    /*private suspend fun insertarEventos(){
        val eventos = listOf(
            Evento(0, 1, 15, "Gol", 0, "Vinicius Jr."),
            Evento(0, 1, 32, "Tarjeta Amarilla", 0, "Gavi"),
            Evento(0, 1, 44, "Gol", 0, "Lewandowski")
        )
        eventos.forEach { db.ligaDao().insertareventos(eventos) }
    }*/

    private suspend fun insertarAlineaciones(){
        ...
        alineaciones.forEach { db.partidoDao().insertarAlineaciones(alineaciones) }
    }
}